...
AppendTo[$Path, FileNameJoin[{$HomeDirectory, "work", "dev", "workspace", "AD", "AD", "Kernel"}]]

BeginPackage["BarrierOption`", {"AD`"}]
...
(*$CDF[mean_, covMat_, x_] := bigPhi[mean, covMat, x]*)
$CDF[mean_, covMat_, x_] := CDF[MultinormalDistribution[mean, covMat], x]
...
doubleKnockOutOutsideBarrierOptionValue[cp_, {sUnd_, sYield_, sVol_}, {bUnd_, bYield_, bVol_}, corr_, intRate_, {t1_, t2_, bLow_, bHigh_}, strike_, t_, ts_, {nMin_, nMax_}] :=
    Module[{t1Local, n, 
        ...
        h2 = defgGeneric[bL, -mu2, t1Local, 0, bVol Sqrt[t1Local], 0];
        wsn = Exp[(mu2 + corr sVol bVol) bp[bL, bH, n] / bVol^2] * 
                ($CDF[{0, 0, 0}, gMat, {cp d1p, e1p, g1}] -
                 $CDF[{0, 0, 0}, gMat, {cp d1p, f1p, g1}] -
                 $CDF[{0, 0, 0}, gMat, {cp d1p, e1p, h1}] +
                 $CDF[{0, 0, 0}, gMat, {cp d1p, f1p, h1}]) -
              Exp[(mu2 + corr sVol bVol) bpp[bL, bH, n] / bVol^2] * 
                ($CDF[{0, 0, 0}, gMat, {cp d1pp, e1pp, g1}] -
                 $CDF[{0, 0, 0}, gMat, {cp d1pp, f1pp, g1}] -
                 $CDF[{0, 0, 0}, gMat, {cp d1pp, e1pp, h1}] +
                 $CDF[{0, 0, 0}, gMat, {cp d1pp, f1pp, h1}]);
        wkn = Exp[mu2  bp[bL, bH, n] / bVol^2] * 
                ($CDF[{0, 0, 0}, gMat, {cp d2p, e2p, g2}] -
                 $CDF[{0, 0, 0}, gMat, {cp d2p, f2p, g2}] -
                 $CDF[{0, 0, 0}, gMat, {cp d2p, e2p, h2}] +
                 $CDF[{0, 0, 0}, gMat, {cp d2p, f2p, h2}]) -
              Exp[mu2 bpp[bL, bH, n] / bVol^2] * 
                ($CDF[{0, 0, 0}, gMat, {cp d2pp, e2pp, g2}] -
                 $CDF[{0, 0, 0}, gMat, {cp d2pp, f2pp, g2}] -
                 $CDF[{0, 0, 0}, gMat, {cp d2pp, e2pp, h2}] +
                 $CDF[{0, 0, 0}, gMat, {cp d2pp, f2pp, h2}]);
        ws = Sum[Evaluate[wsn], {n, nMin, nMax}];
        wk = Sum[Evaluate[wkn], {n, nMin, nMax}];
        value = Exp[-intRate (ts - t)] cp (sUnd Exp[-sYield t] ws - strike Exp[-intRate t] wk)
    ]
